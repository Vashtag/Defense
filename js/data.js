...
const DEFAULT_QUESTIONS = [
  ...
  // ── Allison (Howard canon) ────────────────────────────────────────────────
  { id: 'q70', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','A-effect','idiotropic'], text: '[Allison — Howard canon] Your intro cites Howard (1982) for SVV as a multisensory estimate, but you use SVV changes as an index of cue reweighting without discussing Howard\'s framework for why the A-effect is specifically informative about vestibular-body cue balance. What is the mechanism?' },
  { id: 'q71', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','visual cues','vector sum'], text: '[Allison — Howard canon] Howard showed that visual polarity, frame, and motion cues all independently contribute to perceived vertical. With your visual backgrounds you have multiple cue types superimposed. What does your vector sum model actually fit — gravity, body, or all visual cues lumped together?' },
  { id: 'q72', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','SVV','vection','ecological validity'], text: '[Allison — Howard canon] Guterman and Allison found the A-effect is smaller for optic flow than static lines, and larger during vection than object-motion. Your static-line SVV may be a poor index of the visual-vestibular dynamics during VR where the participant is experiencing vection-like optic flow. Can you defend SVV as the right measure?' },
  { id: 'q73', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','individual differences','Witkin','visual dependence'], text: '[Allison — Howard canon] Howard documented substantial individual differences in visual dependence on SVV, dating back to Witkin and Asch (1948). Yet you treat visual dependence as an experimental variable measured indirectly through cue weights, not as a trait you screened for. How do you know your individual differences aren\'t just the classic Witkin visual dependence dimension?' },
  { id: 'q74', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','SVC','cybersickness','aftereffect'], text: '[Allison — Howard canon] The subjective vertical conflict (SVC) account of cybersickness is based directly on Howard\'s framework and is used in Palmisano et al.\'s (2024) work. SVC predicts sickness when visual SV conflicts with vestibular-somatosensory SV. But you measure SVV aftereffects outside VR, not during it. How do SVV aftereffects measure the conflict that SVC predicts drives sickness during VR?' },
  { id: 'q75', category: 'Committee — Dr. Allison',   difficulty: 'hard',   tags: ['Allison','Howard','head motion','vection','confound'], text: '[Allison — Howard canon] Allison, Howard & Zacher (1999) showed that head motion modulates roll vection and illusory self-tilt. Echo VR is an active game — participants make many head movements. ADR1FT is passive. Did head-movement differences between the two games confound your comparisons across Experiments 1–3?' },
...
Defensible response: within-subjects hardware was constant, so VAC cannot explain the individual-difference clustering. However, it is a legitimate uncontrolled variable; a future iteration should use a varifocal display (Padmanaban et al. 2017) or dynamic focus cues to isolate the vestibular reweighting effect from oculomotor fatigue. Also worth noting: the Quest 2's fixed IPD setting may have introduced additional vergence offset for participants whose IPD differed from the default, compounding the standard VAC with a static horizontal offset — a specific methodological point Allison is well-positioned to raise.`,
  ...
  'q70': `Howard (1982) established that the A-effect occurs because the idiotropic vector (Mittelstaedt, 1983) — a body-axis-referenced signal — biases the SVV toward the body axis when the body is tilted. The magnitude of the A-effect at a given tilt angle indexes the relative weight of the idiotropic (body/somatosensory) signal versus the otolith (gravitational) signal. In the upright condition, they are aligned and there is no A-effect. In RSD, the body axis is perpendicular to gravity and the A-effect magnitude is the angular bias toward the body axis. A post-VR reduction in A-effect (SVV moving toward gravity, negative shift) specifically indicates the idiotropic weight decreased and/or the gravitational weight increased. The vector sum model (Mittelstaedt, 1983; Dyde et al., 2006) formalizes this: gravity is normalized to 1, and the body weight is a free parameter fitted to minimize the residuals across body orientations and visual backgrounds. A decrease in the fitted body parameter after VR is the formal operationalization of the A-effect reduction Howard (1982) would have described verbally.`,

  'q71': `The vector sum model as implemented by Dyde et al. (2006) and used in my dissertation treats the visual component as a single vector whose direction is determined by the visual background orientation (the angular rotation of the room image) and whose length (weight) is a free parameter. It does not decompose the visual contribution into polarity versus frame versus motion sub-cues. This is a limitation: the fitted "vision" weight lumps together all sources of visual orientation information in the background, including both the frame (room alignment) and the polarity (depicted upright of room contents). Howard and Childerson (1994) showed these are additive, so the fitted visual weight in my model is an aggregate that I cannot decompose further without independent manipulation of polarity and frame. A follow-up using backgrounds with consistent frame orientation but reversed polarity (inverted room images) would allow that decomposition.`,

  'q72': `Guterman and Allison (2019) raise a genuine concern about the ecological validity of static-line SVV as a measure of cue weighting during vection. The SVV I measure is a post-exposure aftereffect, not an in-VR measure, so it does not require ecological equivalence to the VR stimulus itself — it indexes the state of the orientation system after the VR session has ended. By that point, vection has terminated and the static-line SVV reflects the residual reweighting persisting from the session. The key question is whether that residual shift meaningfully indexes what happened during VR. The Gaussian relationship between ΔSVV and FMS (R²=0.41) suggests it does, because participants who showed the largest shifts reported the lowest sickness — a relationship that would be arbitrary if ΔSVV were just noise. The Guterman and Allison (2019) finding that vection amplifies the A-effect would actually predict a larger pre-VR A-effect in a subset of participants who experienced strong vection, and a larger post-vection recovery toward baseline — which maps onto the ΔSVV I observe. The two findings are consistent, not contradictory. The cleanest resolution is to measure SVV during VR (in-HMD), which I acknowledge as the critical next experiment.`,

  'q73': `The Witkin visual dependence dimension, as measured by rod-and-frame test or embedded figures, indexes a stable trait associated with reliance on visual frames for spatial orientation. Howard (1982) reviewed this literature and noted that highly visually dependent individuals show larger A-effects and are more susceptible to visual frame tilts. My dissertation separates trait from state: I measure change in cue weighting from pre- to post-VR (ΔSVV, ΔPU), not absolute cue weight at baseline. Even if my participants varied in baseline visual dependence (a limitation I acknowledge — I did not administer RFT), the Δ-measure controls for that stable trait variance. What predicts cybersickness in my data is the magnitude of the shift from baseline, not the baseline itself. Josupeit (2024) has recently shown that static RFT measures of visual dependence have limited predictive validity for cybersickness, while dynamic reweighting capacity is more informative — my findings are consistent with that conclusion.`,

  'q74': `SVC predicts that the discrepancy between visual and vestibular-somatosensory estimates of vertical during VR drives sickness. My SVV aftereffect does not measure that discrepancy directly — it measures a consequence of prolonged unresolved discrepancy. The logic is that individuals who experienced a large unresolved subjective vertical conflict during VR will show the largest post-exposure recalibration (ΔSVV), while those who successfully reweighted in real time will show smaller post-exposure shifts because their internal model has already adapted during VR. The Gaussian relationship between ΔSVV and FMS is consistent with an optimal reweighting account: zero ΔSVV means no adaptation occurred (either because conflict was too small or because adaptation was impossible), and large ΔSVV means conflict-driven recalibration (protective if well-timed, distressing if reactive). This is not the same as directly measuring SVC during VR, and I acknowledge that the aftereffect-SVC link is inferential rather than direct. The cleanest direct test would co-register in-VR SVV with FMS.`,

  'q75': `Yes, this is a potential confound I should have and do acknowledge. Echo VR involves active head movements during gameplay (tracking opponents, looking around the arena), while ADR1FT is a passive traversal game with more constrained head movement. Allison, Howard and Zacher (1999) showed that head motion modulates vection strength and illusory self-tilt. If active head movements produce larger visual-vestibular conflicts during gameplay, Echo VR participants in Experiments 1 and 2 may have experienced stronger conflict episodes than ADR1FT participants in Experiment 3, contributing to differences in post-exposure cue weight shifts. I did not record head movement telemetry in any experiment (a methodological gap), so I cannot quantify this confound. In the published versions of these chapters I will treat the cross-game comparison as descriptive only, and frame the Echo VR versus ADR1FT difference as a factor that motivates a within-subject experimental design using both games and the same participants.`,
...
const DEFAULT_DICTIONARY = [
  ...
  { term: 'A-effect (Aubert Effect)', category: 'Core Concept', definition: 'At body tilts greater than ~60° in roll, the subjective visual vertical (SVV) biases toward the body\'s longitudinal axis rather than toward gravity. Named after Aubert (1861). Mechanism: the idiotropic vector (Mittelstaedt, 1983) gains relative weight over the otolith graviceptive signal as the otolith becomes less reliable at large tilts. At 90° body tilt (the right-side-down posture used in the dissertation), the A-effect magnitude is typically 8–20° depending on the individual and visual context — a large, reliable signal that makes the 90° RSD posture maximally sensitive to the body cue weight parameter in the vector sum model. A post-VR reduction in A-effect (SVV shifting toward gravity) operationalises a decrease in the idiotropic/body cue weight — the key index of Flexible Adapter reweighting.' },
  { term: 'E-effect (Müller Effect)', category: 'Core Concept', definition: 'At small body tilts of less than ~30° in roll, the SVV biases away from the body axis — in the direction of gravity. Named after Müller (1916). Mechanism: at small tilts the otolith graviceptive signal remains highly reliable and dominates over the weaker idiotropic vector, pulling the percept toward true vertical. The E-effect and A-effect define a non-monotonic function of body tilt: as tilt increases from 0° the SVV first overshoots gravity (E-effect, ~10–30°), then crosses through zero error (~40–60°), then biases toward the body axis (A-effect, >60°). Understanding this crossover explains why 90° RSD was chosen for the dissertation — it sits squarely in the A-effect zone where body-cue weighting is maximal and most detectable.' },
  ...
  { term: 'Neural Mismatch Theory', category: 'Theory', definition: 'A contemporary evolution of classical sensory conflict theory (Palmisano et al., 2024). Proposes that cybersickness occurs when the brain detects a discrepancy between current multisensory inputs (visual, vestibular, somatosensory) and the internal models of movement stored in long-term memory. More nuanced than Reason (1978) in that it specifies the mismatch as one between live sensory input and learned, memory-based predictions — emphasising the role of prior experience and individual differences in internal model fidelity.' },
...
const CHAPTERS = [
  {
    ...
    subtitle: 'Why reweighting, why VR, why now',
    description: 'Establishes the gap in cybersickness literature, introduces sensory reweighting as the core mechanism, and positions the vector sum model (PU = v⃗ + b⃗ + g⃗) against competing frameworks (MLE, Bayesian, Observer models).',
    color: '#6366f1',
    categories: ['Introduction & Core Argument', 'Reweighting & Modelling'],
    committeeAngles: 'Barnett-Cowan will probe the cumulative argument and theoretical novelty. Allison will compare the vector sum to engineering models.',
  },
  {
    id: 'ch2',
    number: 2,
    title: 'Experiment 1 — SVV & VR Exposure',
    subtitle: 'Gravity-ward SVV shift as a reweighting signature',
    description: 'Participants completed SVV before and after low/high-intensity VR (right-side-down posture). Key result: significant post-VR gravity-ward shift (F(1,18) = 8.07, p = .012, η²p = .34), interpreted as down-weighting of the vestibular/body vector and up-weighting of gravity.',
    color: '#22c55e',
    categories: ['Experiment 1 — SVV'],
    committeeAngles: 'Allison: display parameters, VOR. Barnett-Cowan: relationship to Chung & Barnett-Cowan (2023). Staines: neural substrate of the SVV shift.',
  },
  {
    id: 'ch3',
    number: 3,
    title: 'Experiment 2 — OCHART & Individual Differences',
    subtitle: 'Flexible Adapters vs Rigid Perceivers',
    description: 'Replicated Exp 1 using OCHART (body-dominant) alongside SVV (gravity-dominant). Null group effect — but k-means clustering revealed two stable clusters: Flexible Adapters (successful reweighting, low FMS) and Rigid Perceivers (failed reweighting, high FMS). Cross-experiment pooling justified by Kruskal-Wallis (p > .07).',
    color: '#f59e0b',
    categories: ['Experiment 2 — OCHART', 'Individual Differences'],
    committeeAngles: 'Barnett-Cowan: null result + non-ergodicity. Niechwiej-Szwedo: eye movements during OCHART. Itier: attentional confounds, field dependence.',
  },
  {
    id: 'ch4',
    number: 4,
    title: 'Experiment 3 — EVS Intervention',
    subtitle: 'Causal test of the reweighting mechanism',
    description: 'Applied bilateral stochastic EVS during VR to reduce vestibular reliability and promote reweighting. Result: EVS worsened cybersickness (p = .001, r = .67) and shortened VR tolerance — opposite to the prediction. Interpreted as EVS adding conflict rather than enabling adaptation.',
    color: '#ef4444',
    categories: ['Experiment 3 — EVS'],
    committeeAngles: 'Allison: EVS waveform parameters, VAC. Staines: EVS cortical mechanism, PIVC disruption. Niechwiej-Szwedo: EVS-induced ocular torsion and OCHART confound.',
  },
  {
    id: 'ch5',
    number: 5,
    title: 'General Discussion',
    subtitle: 'Contributions, limitations, and future directions',
    description: 'Integrates findings across three experiments, defends the converging-operations argument, situates Flexible/Rigid typology within field-dependence and TBW traditions, acknowledges limitations (small N, recumbent posture, stochastic EVS), and outlines future work (individualized EVS, varifocal displays, EEG/TMS follow-up).',
    color: '#a855f7',
    categories: ['Contributions & Implications', 'Limitations & Future Work'],
    committeeAngles: 'Barnett-Cowan: theoretical take-home with mixed results. Itier: cognitive predictors, EEG markers. All committee members may return to weaknesses here.',
  },
  {
    id: 'ch0',
    number: null,
    title: 'Cross-Cutting: Methods & Design',
    subtitle: 'Questions that span all experiments',
    description: 'Methodological choices that apply across the dissertation: right-side-down posture rationale, FMS vs SSQ, within-subjects design, sample size justification, and pooling strategy.',
    color: '#06b6d4',
    categories: ['Methodology & Design'],
...
const BRAIN_REGIONS = [
  ...
  // Primary cortex
  { id:'v1',   label:'V1',              x:618, y:290, lx:12,  ly:0,   anchor:'start',  color:'#06b6d4', desc:'Primary visual cortex (striate cortex, occipital pole). Receives LGN input; performs edge/orientation/spatial-frequency detection. Distributes to dorsal (V2→MT) and ventral (V2→V4) streams. Deactivated by GVS (reciprocal visuo-vestibular inhibition, Brandt et al. 1998).' },
  { id:'s1',   label:'S1',             x:356, y:82,  lx:0,   ly:-13, anchor:'middle', color:'#14b8a6', desc:'Primary somatosensory cortex (postcentral gyrus). Receives proprioceptive and tactile signals via thalamus. Staines\' lab: DLPFC→S1 top-down gating modulates body-cue input by task relevance — the cortical mechanism of body-cue down-weighting in Flexible Adapters (Bolton & Staines 2011).' },
  { id:'mc',   label:'Motor Cortex',   x:272, y:88,  lx:0,   ly:-13, anchor:'middle', color:'#f97316', desc:'Primary motor cortex (precentral gyrus). Generates motor commands; sends efference copy to cerebellum for forward prediction. Efference copy allows the brain to distinguish self-generated from external motion — key for distinguishing vection from actual movement in VR.' },
  // Visual streams
  { id:'v2',   label:'V2',             x:594, y:248, lx:12,  ly:0,   anchor:'start',  color:'#22c55e', desc:'Secondary visual cortex. Processes orientation, colour, and depth. Distributes to dorsal (→MT via V3) and ventral (→V4) streams. Thick/thin/pale stripe architecture separates M and P pathway projections.' },
  ...
  { id:'pfc',  label:'DLPFC / PFC',    x:175, y:162, lx:-13, ly:0,   anchor:'end',    color:'#ef4444', desc:'Dorsolateral Prefrontal Cortex. Top-down gating of sensory input. DLPFC→S1 pathway modulates body-cue down-weighting (Staines et al. 2002; Bolton & Staines 2011). Receives conflict signal from ACC. Attentional set from PFC may modulate PIVC activity (Frank et al. 2021).' },
  { id:'acc',  label:'ACC',            x:233, y:102, lx:0,   ly:-13, anchor:'middle', color:'#ef4444', desc:'Anterior Cingulate Cortex. Conflict monitoring and prediction error signalling. Receives mismatch signal when vestibular and visual inputs disagree — part of the neural circuit generating the discomfort/nausea of cybersickness. Connects conflict detection to prefrontal top-down modulation.' },
];
